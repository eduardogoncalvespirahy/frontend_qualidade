...
@Component({
  ...
  styleUrl: './package-types.component.css',
})
export class PackageTypesComponent implements OnInit {
  private readonly service = inject(PackageTypesService);
  private readonly modalService = inject(ModalService);

  readonly items = signal<PackageTypes[]>([]);
  ...
  private load(): void {
    ...
    this.service.getAll(1000, 1).subscribe({
      next: (res) => {
        this.items.set(this.unwrap<PackageTypes>(res));
        this.loading.set(false);
      },
      error: (err) => {
        this.error.set('Erro ao carregar tipos de pacotes');
        this.loading.set(false);
      },
    });
  }
  ...
  async novo(): Promise<void> {
    ...
    this.service.create({ nome: v.nome, descricao: v.descricao, status: v.status }).subscribe({
      next: () => {
        this.success.set('Tipo de pacote criado com sucesso.');
        this.load();
      },
  ...
  async editar(item: PackageTypes): Promise<void> {
    this.clearFeedback();
    const ref = this.modalService.openComponent(FormComponent, {
    ...
    this.service.update(item.id, { nome: v.nome, descricao: v.descricao, status: v.status }).subscribe({
      next: () => {
        this.success.set('Tipo de pacote atualizado com sucesso.');
        this.load();
      },
  ...
  async excluir(item: PackageTypes): Promise<void> {
    this.clearFeedback();
    ...
    this.service.delete(item.id).subscribe({
      next: () => {
        this.success.set('Tipo de pacote excluído.');
        this.load();
      },
  ...
  trackById(index: number, item: PackageTypes): string {
    return item.id;
  }
}
