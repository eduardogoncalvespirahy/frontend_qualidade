...
export class BreakMachineComponent implements OnInit {
  ...
  load(): void {
    ...
    this.fetchAllPages<BreakMachine>(
  ...
  private matchSituacao(b: BreakMachine, f: Situacao, now: Date): boolean {
    switch (f) {
      case 'all':
        return true;
      case 'ativa':
  ...
  private dentroPeriodo(b: BreakMachine, de: string, ate: string): boolean {
